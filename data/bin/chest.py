...
class Chest:
          ...
          if achar.rect.top == self.rect.bottom:# and its touching the bottom of chest and chest label is unopened
            if keys[pygame.K_e]: #user hits e
              if self.isopen < 1: #delete this and do self.isopen = ``
                self.isopen = 1
                chest_reward = (self.chest_reward(agame.chest_rewards, agame.item_list, (self.x/32, self.y/32),agame))
                amessage.show +=1
  ...
  def chest_reward(self,llist,ilist,coord,game):
    for chest in llist:  #((3,3) , 0)
      if game.currentLevel == chest[2]:
        if chest[0] == coord:
